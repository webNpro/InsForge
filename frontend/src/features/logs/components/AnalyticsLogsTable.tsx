...
export function AnalyticsLogsTable({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      {/* Status bar */}
      <div className="px-4 py-2 bg-gray-50 border-b text-xs text-gray-600 flex justify-between items-center">
        ...
        {isLoadingMore && (
          <span className="text-gray-500">Loading older logs...</span>
        )}
      </div>
      ...
      >
        <div className="font-mono text-sm">
          ...
          {logs.map((log, index) => {
            ...
            return (
              <div key={uniqueKey} className="border-b border-gray-100 hover:bg-gray-50">
                <div 
                  className={cn(
                ...
                >
                  ...
                  <div className="flex-shrink-0 w-48 bg-gray-100 px-3 py-1 rounded mr-3 text-gray-700 text-xs">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2">
                      ...
                      {log.body?.status_code && (
                        <Badge 
                          variant="outline" 
                          className={cn("text-xs border-0", getStatusCodeColor(log.body.status_code))}
                        >
                          {log.body.status_code}
                        </Badge>
                      )}
                    ...
                    <div className="ml-48 pl-3">
                      ...
                      {log.body?.log_type === 'request' && (
                        <div className="space-y-1 text-xs text-gray-600 mb-3">
                          {log.body.ip && (
                            <div>
                              <span className="font-medium text-gray-700">IP:</span> {log.body.ip}
                            </div>
                          )}
                          {log.body.user_agent && (
                            <div>
                              <span className="font-medium text-gray-700">User Agent:</span> {log.body.user_agent}
                            </div>
                          )}
                          {log.body.path && (
                            <div>
                              <span className="font-medium text-gray-700">Path:</span> {log.body.path}
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      {log.body?.error && (
                        <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded text-xs">
                          ...
                          <div className="text-red-700">{log.body.error}</div>
                          {log.body.stack && (
                            <pre className="mt-2 text-red-600 text-xs whitespace-pre-wrap overflow-x-auto">
                              {log.body.stack}
                            </pre>
                          )}
                        </div>
          ...
          {!hasMore && logs.length > 0 && (
            <div className="px-4 py-3 text-center text-gray-500 text-xs bg-gray-50">
