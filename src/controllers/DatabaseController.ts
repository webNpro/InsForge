...
export class DatabaseController extends BaseController {
  ...
  constructor() {
    ...
    this.postgrestUrl = process.env.POSTGREST_BASE_URL || 'http://localhost:5430';
  }

  /**
   * Forward database requests to PostgREST
   */
  forwardToPostgrest = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const targetUrl = `${this.postgrestUrl}${targetPath}`;
      ...
      this.success(res, responseData, response.status);

      // Log the activity
      await this.dbManager.logActivity(req.method, tablename, wildcardPath || 'table', {
        query: req.query,
        user_id: req.user?.id,
      });
    } catch (error) {
